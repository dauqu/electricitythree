...
export default function Page() {
  ...
  async function getAllRepo() {
    ...
    try {
      await axios
        .get("https://electricitythree.vercel.app/api/sms")
        .then((response) => {
              ...
              data?.map((repo, index) => (
                ...
                >
                  <span className="b bg-slate-800 flex p-2">
                    <span className="uppercase mr-5"> SMS:</span>
                    <span className=""> {repo?.body}</span>
                  </span>
                  <span className="b bg-slate-800 flex p-2">
                    <span className="uppercase mr-5"> Sender:</span>
                    <span className=""> {repo?.sender}</span>
                  </span>
